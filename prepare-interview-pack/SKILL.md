...
## Verify Basic Company Context in Every Mode

Before company-specific positioning or STARS analysis, read [references/company-research.md](references/company-research.md) and complete basic company verification in Sprint, Standard, and Deep modes. Deadline changes research depth, not whether this step occurs. Verify the exact name against the original JD or image before searching; do not silently trust OCR or a prior draft.

Cover identity and brand relationships, scale and business model, recent developments, conflicting evidence, and interview implications. Use current authoritative sources where available. Record actual searches, sources read, dates, findings, and unresolved gaps in `company-research.json`. A heading, generic STARS hypothesis, or unsearched `unknown` is not research completion. Attach each supported research topic to the exact source image/block/paragraph and a short evidence excerpt. Distinguish publication dates from page updates. Before publication, manually verify that cited excerpts really occur in the pointed-to sources; a record naming the wrong screenshot is not adequate attribution. If browsing is explicitly prohibited or tools are unavailable, document that exception and continue with clearly bounded supplied evidence; lack of public results must not block the candidate's answer library.

...
Before delivery or publication, run `python3 scripts/validate_pack.py <handbook.md> --mode <sprint|standard|deep>`. When a complete resume was read, also pass `--resume-projects <manifest.json|manifest.txt>`. Treat every reported error as a required revision. Review warnings explicitly and fix those that are relevant; never claim the artifact passed if the validator did not run successfully.
